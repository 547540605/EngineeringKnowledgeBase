...
> [!WARNING]
> **规划成功不等于执行成功**：
> 1. `plan()` 成功仅代表在当前的静态规划场景（Planning Scene）中根据当前已知碰撞模型找到了一条满足关节限位与无碰撞约束的几何路径；
> 2. 真实物理环境中可能存在动态未知障碍物、驱动器过载、机械臂急停（E-Stop）或控制器跟踪误差超限。因此工程实战中**必须严格校验 `execute()` 返回状态码**，并设计完整的重试、回退与故障保护机制；
> 3. 仿真与纯教学软件中的运动平滑绝非物理系统功能安全（Functional Safety）的证明，真实工业机器人工作站必须接入安全门联锁、激光安全扫描仪与急停硬件回路。
