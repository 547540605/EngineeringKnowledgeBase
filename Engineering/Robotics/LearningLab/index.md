...
> **说明**：原有 75 个交互式 HTML 与 11 个独立 JS 物理仿真脚本将在**阶段 2** 正式同步迁入本目录；以下为实验区收录的功能清单。
...
* **ROS 2 工程实战看板与进阶路线** (`ros2/00-progress.html`)：六大工程阶段（数字孪生 $\to$ 3R 逆解 $\to$ 轨迹规划 $\to$ MoveIt 2 $\to$ 在线闭环 $\to$ 工位工程化）。
...
* **ros2_control、轨迹 Action 与 Fake Hardware** (`ros2/13-ros2-control-and-fake-hardware.html`)：Controller Manager、Action 状态机与硬件接口契约。
