...
| 类别 | 控制关系／信号方向 |
| --- | --- |
| A 显式力控制 | 测量力与目标/命令力比较 → 力误差闭环 |
| B 刚度控制 | 位移偏差 → 恢复力（弹簧行为） |
| C 阻抗控制 | 运动状态 → 反作用力的动态关系 |
| D 导纳／顺应控制 | 外力测量 → 速度或位置响应 |
| E 隐式力控制 | 电流、关节力矩及模型 → 推断力 |
| F 自然导纳控制 | 接触力与虚拟弹簧、阻尼和吸引点共同作用 → 加速度 |

这些类别并非互斥：实际系统可能组合刚度、阻尼、力反馈和运动内环；同一名称在不同文献中的数学结构也可能不同。
