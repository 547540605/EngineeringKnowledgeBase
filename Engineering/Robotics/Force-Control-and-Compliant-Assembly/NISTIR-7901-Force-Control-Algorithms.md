...
报告在 Section V 中将 A–F 作为并列的六类主要力控算法。下面按原文顺序介绍；A 的框图和 PI 仿真是对该方法的展开示例，不表示 A 高于或统领 B–F。
...
区分这些方法时，重点看**力、位置/位移、速度和加速度之间建立了什么关系，以及运行时是否把外部力传感器读数放进反馈回路**。这些名称描述控制关系，不保证不同厂商或论文的实现完全相同；下面按报告的概述解释，并把便于理解的直觉单独说明。
...
显式力控制 A 的识别重点是：外部测得的力是否直接进入反馈回路并参与误差调节。刚度、阻抗、导纳、隐式力和自然导纳是相关但不同的控制关系，实际系统也可能组合它们；各类方法的边界见上面的 A–F 并列介绍。
